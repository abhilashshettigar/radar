...
## Opening MCP to AI agents without authentication

When Radar runs with authentication enabled (`--auth-mode=proxy` or `oidc`), both `/mcp` and `/mcp-readonly` require a session, which AI agent CLIs can't easily complete. To let agents reach the **read-only** endpoint without a browser/OIDC login, start with:

```bash
radar --auth-mode=oidc ... --mcp-open
```

`--mcp-open` exempts only `/mcp-readonly` from authentication; the write-capable `/mcp` endpoint always requires auth. The open endpoint runs with the process's kubeconfig/ServiceAccount identity (no per-user RBAC filtering) and should only be reachable on a trusted/loopback listener. It is ignored in Radar Cloud mode, where the tunnel identity boundary stays authoritative.
...
- **`--mcp-open` bypasses auth for `/mcp-readonly` only** — the open endpoint runs with the kubeconfig/ServiceAccount identity (no per-user RBAC) and exposes logs, minified specs, events, ConfigMap keys, and redacted env values to anyone who can reach the listener. Keep it loopback or behind a trusted reverse proxy; never combine it with `--listen-address=0.0.0.0` on an untrusted network.
